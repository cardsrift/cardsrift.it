			...
			<div class="swiper hero-slider !z-[6] max-lg:h-[600px] lg:h-full">
				...
				<div class="swiper-wrapper h-full">
					<?php foreach($hero_slides as $key => $slide) : ?>
						<?php
							// I campi immagine possono mancare (es. slide di tipo video): guardie ?? per evitare warning
							$image_mobile_url = $slide['slide_mobile']['url'] ?? '';
							...
							<?php if ($type === 'img') : ?>
								...
								<a class="swiper-slide overflow-hidden block h-full" href="">
									<picture class="block h-full w-full relative ">
											<source media="(max-width: 1200px)" srcset="<?php echo esc_url($image_mobile_url); ?>">
									...
									<div class="swiper-slide overflow-hidden h-full flex items-center justify-center bg-black">
										...
										<?php if (!empty($slide['video_mobile']['url'])): ?>
											<video class="heroVideo object-cover object-center pointer-events-none h-full w-full max-lg:block lg:hidden" autoplay muted loop>
												<source src="<?= esc_url($slide['video_mobile']['url']); ?>">
											</video>
										<?php endif; ?>
									</div>
