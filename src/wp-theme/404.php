			...
			<div class="min-h-[calc(100vh_-_290px)] py-16 flex flex-col items-center justify-center text-center">
				<div class="flex flex-col items-center gap-8 w-full max-w-md">
					...
					<div class="flex flex-wrap items-center justify-center gap-2">
						<a class="cr-chip no-underline hover:border-th-acc hover:text-th-acc transition-colors" href="<?= esc_url(home_url('/')); ?>"><?php esc_html_e('Home', 'cardsrift'); ?></a>
						<?php foreach ($games as $slug => $label) : ?>
							<a class="cr-chip no-underline hover:border-th-acc hover:text-th-acc transition-colors" href="<?= esc_url(home_url('/' . $slug . '/')); ?>"><?= esc_html($label); ?></a>
						<?php endforeach; ?>
					</div>
