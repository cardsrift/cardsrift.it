...
import { GLOBAL_VARS, MOBILE_SCROLLER_SELECTOR } from './constants';
...
export const scaledVal = (rem) => (windowWidth / 192) * rem;
...
export const setScrollValues = () => {
	const isMob = isTouchDevice();
	const requiredPage = document.body.classList.contains('body--gpe_replenishment_mod');
	if (!isMob || !requiredPage) return {};
	return {
		scroller: MOBILE_SCROLLER_SELECTOR,
		pinType: 'fixed',
	};
};
